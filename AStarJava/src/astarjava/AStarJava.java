 ...
 */    
    ...
    public static double heuristic(int x, int y, int gx, int gy) {    
        ...
        // Add Starting Tile to custom
        // While Target not found
        // Select next step with least cost and add to custom
        // return custom
        double customHueristic;
        //redefine manhattan as previously mentioned for later use
        manhattan = Math.abs(gx - x) + Math.abs(gy - y);
        
        //Distance between current state and goal state
        int x1 = x - gx;
        int y1 = y - gy;
        //Distance between start state and goal state
        int x2 = startX - gy;
        int y2 = startY - gy;
        
        double modifier = Math.abs(x1 * y2 - x2 * y1);
        customHueristic = manhattan + (modifier * 0.001);
        return customHueristic;
        }
    }
